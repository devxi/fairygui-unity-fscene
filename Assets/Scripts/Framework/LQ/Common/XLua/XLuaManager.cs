...
public class XLuaManager : MonoSingleton<XLuaManager>
{
    ...
    public static byte[] CustomLoader(ref string filepath)
    {
        ...
        if (AssetBundleConfig.IsEditorMode)
        {
            ...
            var bytes = GameUtility.SafeReadAllBytes(scriptPath);
            if (bytes != null)
                Logger.Log("Load lua script size:" +  bytes.Length + "B path:" + scriptPath);
            else
                Logger.LogError("找不到lua脚本:" + scriptPath);
            return bytes;
        }
#endif
        ...
        Debug.LogError("找不到指定lua脚本:" + filepath);
        return null;
    }
